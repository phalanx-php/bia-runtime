...
class CodeReferencesCommand implements Scopeable, DescribesCommand
{
    ...
    public function __invoke(CommandContext $ctx): int
    {
        ...
        if (!$ctx->options->flag('all') && !CodeCommandInput::hasAnyOption($ctx, 'kind', 'name', 'file', 'context')) {
            $message = 'Provide --kind, --name, --file, --context, or --all before listing project references.';
    ...
    public static function commandConfig(): CommandConfig
    {
        return new CommandConfig(
            description: 'Search project code references by kind, name, file, or context',
            arguments: [
            ...
            examples: [
                ...
                'dory code references app --name=helper --json',
            ],
        );
    }
    ...
    private static function referenceLine(ReferenceRecord $reference): string
    {
        ...
        return "{$reference->kind} {$reference->name} {$context} {$file}:{$line}";
    }
}
